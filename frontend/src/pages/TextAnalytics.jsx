/**
 * TEXT/SMS Campaigns Analytics/KPIs Page
 */
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-right font-semibold text-gray-700">Prospects</th>
              <th className="px-4 py-3 text-right font-semibold text-gray-700">Delivered</th>
              <th className="px-4 py-3 text-right font-semibold text-gray-700">Responses</th>
              ...
              <th className="px-4 py-3 text-right font-semibold text-gray-700">Resp→Lead Rate</th>
          ...
          <tbody>
            {data.map((row, idx) => (
              <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right">{formatNumber(row.totalProspects, 0)}</td>
                <td className="px-4 py-3 text-right">{formatNumber(row.totalMessagesDelivered, 0)}</td>
                <td className="px-4 py-3 text-right">{formatNumber(row.totalResponses, 0)}</td>
                ...
                <td className="px-4 py-3 text-right">{formatPercent(row.responseToLeadRate)}</td>
...
const TextAnalyticsPage = () => {
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      const response = await campaignsApi.getFilterOptions();
      setFilterOptions({
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const response = await campaignsApi.getAnalytics(params);
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">TEXT/SMS Analytics</h1>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <KpiCard
            ...
            value={formatNumber(overall.totalProspects, 0)}
            subtitle={`${overall.totalCampaigns || 0} campaigns`}
            color="blue"
          />
          <KpiCard
            title="Messages Delivered"
            value={formatNumber(overall.totalMessagesDelivered, 0)}
            color="blue"
          />
          <KpiCard
            title="Total Responses"
            value={formatNumber(overall.totalResponses, 0)}
          ...
          <KpiCard
            ...
            subtitle={`${formatPercent(overall.responseToLeadRate)} of responses`}
            color="purple"
          />
          <KpiCard
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mt-4">
          ...
          />
          <KpiCard
            title="Response to Lead Rate"
            value={formatPercent(overall.responseToLeadRate)}
            subtitle="Leads / Responses"
            color="purple"
          />
          <KpiCard
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-4">
          <KpiCard
            ...
            subtitle="Average of campaigns"
            color="orange"
          />
          <KpiCard
            title="Avg Cost Per Contract"
            value={formatCurrency(overall.avgCostPerContract)}
            subtitle="Average of campaigns"
            color="orange"
          />
          <KpiCard
            title="Avg Proceeds/Contract"
            value={formatCurrency(overall.avgClosingProceedsPerContract)}
            subtitle="Total Proceeds / Closed"
            color="green"
          />
          <KpiCard
            title="Total Margin"
            ...
            color={(overall.totalMargin || 0) >= 0 ? 'green' : 'red'}
          />
          <KpiCard
            title="ROAS"
