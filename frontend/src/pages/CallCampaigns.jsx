...
import { callCampaignsApi } from '../services/api';
import { 
  formatCurrency, 
  formatPercent, 
  formatDate, 
...
const columns = [
  ...
  { key: 'contract_sent_to_contract_closed_rate', label: 'Close Rate', sortable: true, render: (v) => formatPercent(v) },
...
const formFields = [
  ...
  { key: 'lead_to_contract_sent_rate', label: 'Lead to Contract Rate (%)', type: 'number', step: '0.01' },
  { key: 'contract_sent_to_contract_closed_rate', label: 'Close Rate (%)', type: 'number', step: '0.01' },
  ...
  { key: 'average_margin_per_contract_2', label: 'Avg Margin/Contract 2', type: 'number', step: '0.01' },
];
...
const CallCampaignsPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await callCampaignsApi.getAll(params);
  ...
  const handleSave = async (formData) => {
    ...
    try {
      if (selectedItem) {
        await callCampaignsApi.update(selectedItem.id, formData);
        toast.success('Record updated successfully!');
      } else {
        await callCampaignsApi.create(formData);
  ...
  const handleConfirmDelete = async () => {
    setSaving(true);
    try {
      await callCampaignsApi.delete(selectedItem.id);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Call Campaigns</h1>
          <p className="text-gray-600">Cold Calling Campaigns</p>
        </div>
