...
const filterConfig = [
  ...
  { key: 'sequence_number', label: 'Sequence', type: 'text' },
...
const formFields = [
  ...
  { key: 'state', label: 'State', type: 'text' },
  { key: 'entity', label: 'Entity', type: 'text' },
  { key: 'sequence_number', label: 'Sequence Number', type: 'text' },
...
const MailCampaignsPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await mailCampaignsApi.getAll(params);
  ...
  const handleSave = async (formData) => {
    ...
    try {
      if (selectedItem) {
        await mailCampaignsApi.update(selectedItem.id, formData);
        toast.success('Record updated successfully!');
      } else {
        await mailCampaignsApi.create(formData);
  ...
  const handleConfirmDelete = async () => {
    setSaving(true);
    try {
      await mailCampaignsApi.delete(selectedItem.id);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Mail Campaigns</h1>
          <p className="text-gray-600">Direct Mail Campaigns</p>
        </div>
